// Tweaks panel — simplified standalone version (no iframe protocol).
import * as React from 'react';

const TWEAKS_STYLE = `
  .twk-panel{width:280px;display:flex;flex-direction:column;
  ...
  .twk-chip{position:relative;appearance:none;flex:1;min-width:36px;height:36px;
...
export function useTweaks<T extends Record<string, any>>(defaults: T): [T, <K extends keyof T>(k: K, v: T[K]) => void] {
...
export function TweakButton({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return (
    <button type="button" className="twk-btn" onClick={onClick}>{children}</button>
  );
}
