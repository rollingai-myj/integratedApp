...
export const VirtualShelfView = forwardRef<VirtualShelfViewHandle, Props>(({ layout, shelfWidthCm }, ref) => {
  ...
  const allLayerHeights = useMemo(() => {
    return layout.map(group =>
      group.layers.map(layer => {
        const maxH = layer.blocks.reduce((max, b) => Math.max(max, b.heightCm), 10);
  ...
  const totalHeight = useMemo(() => {
    const maxLayers = allLayerHeights.reduce((max, lh) => {
      const sum = lh.reduce((s, h) => s + h + WIRE_SHELF_H, 0);
  ...
  const colorMap = useMemo(() => {
    ...
    layout.forEach(g => g.layers.forEach(l => l.blocks.forEach(b => {
  ...
  return (
    <Tooltip open={open} onOpenChange={(o) => { if (!o) setClicked(false); }}>
      ...
      <TooltipContent side="top" className="max-w-[240px]" onPointerDownOutside={() => setClicked(false)}>
        {block.isNewListing && (
          <p className="text-xs font-semibold text-amber-600 mb-1">本次上架</p>
        )}
