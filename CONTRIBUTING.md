...
1. **`main` 分支永远是"能用的版本"**——不允许任何人直接往 main 提交代码（仓库分支保护规则强制）
2. **每个任务用一根独立分支**——任务之间互不打架
3. **改动通过 Pull Request 进入 main**——任何改动都必须经过 PR
4. **小步快跑**——一个 PR 解决一件事，不要一个 PR 改 50 个文件

### 评审规则的当前阶段

当前是 **solo + AI 工作流阶段**（团队主要由 1 个业务负责人 + AI 工具组成），评审规则按下面这样配置：

| 项目 | 当前 | 团队扩大后 |
|---|---|---|
| PR 是否必须 | ✅ 是 | ✅ 是 |
| 是否必须有人批准才能合并 | ❌ 否（允许 PR 作者自合并） | ✅ 至少 1 人批准 |
| 是否能直接 push 到 main | ❌ 否 | ❌ 否 |
| 是否要把 review 里的评论解决掉才能合并 | ✅ 是 | ✅ 是 |

"允许自合并" 不等于"可以乱合并"——合并前你必须：

1. **打开 PR 的 "Files changed" 标签页**，从头到尾把 diff 看一遍
2. **跑一遍 "怎么验证" 里写的步骤**，确认改动是对的
3. **看截图/录屏**，确认 UI 改动符合预期
4. **如果心里没底**，叫 AI 工具帮你看一遍 diff（"帮我评审一下这个 PR，找出潜在问题"）

什么时候改成"必须有人批准"？

- 加入第二个固定贡献者后
- 或者改成 "强制 CI 检查通过" + 0 人批准（用 CI 替代人工评审，等做了 M5 加 CI 之后）

到时候改一行配置即可：
```bash
gh api -X PUT repos/rollingai-myj/integratedApp/branches/main/protection \
...
- 不需要别人批准，但**合并前你必须自检 diff + 跑一遍验证步骤**
- 如果不确定，叫 AI 工具帮你看一遍（"帮我评审这个 PR，找出潜在问题"）
